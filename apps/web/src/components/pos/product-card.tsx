...
function StockBadge({ stock }: { stock: number }) {
  ...
  if (stock <= 2) return <span className="text-xs font-semibold text-red-500">Only {stock} left!</span>;
  if (stock <= 5) return <span className="text-xs font-semibold text-amber-500">{stock} left</span>;
  return <span className="text-xs text-slate-300">{stock} available</span>;
}
