...
function computeCategorySales(orders: Order[]): { category: string; count: number }[] {
  ...
  for (const order of orders) {
    for (const item of order.items) {
      if (item.product.type !== "sale") continue;
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="rounded-xl bg-blue-50 border border-blue-100 shadow-sm px-5 py-4">
                ...
                <p className="text-xs text-blue-400">{dayStats.count} orders</p>
              </div>
              ...
              <div className="rounded-xl bg-emerald-50 border border-emerald-100 shadow-sm px-5 py-4">
                ...
                <p className="text-xs text-emerald-400">{dayStats.itemsSold} item{dayStats.itemsSold !== 1 ? "s" : ""} sold</p>
              </div>
              <div className="rounded-xl bg-violet-50 border border-violet-100 shadow-sm px-5 py-4">
                ...
                <p className="text-xs text-violet-400">{dayStats.itemsRented} item{dayStats.itemsRented !== 1 ? "s" : ""} rented</p>
              </div>
            </div>
            ...
            {dayStats.count > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                <div className="rounded-xl bg-emerald-50 border border-emerald-100 shadow-sm px-5 py-4">
                  ...
                  <p className="text-xs text-emerald-400">{dayStats.cashOrders} order{dayStats.cashOrders !== 1 ? "s" : ""}</p>
                </div>
                <div className="rounded-xl bg-indigo-50 border border-indigo-100 shadow-sm px-5 py-4">
                  ...
                  <p className="text-xs text-indigo-400">{dayStats.qrOrders} order{dayStats.qrOrders !== 1 ? "s" : ""}</p>
                </div>
              </div>
            )}
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-4 gap-3">
                  {dayCategorySales.map(({ category, count }) => (
                    <div key={category} className="rounded-xl bg-white border border-slate-100 shadow-sm px-4 py-3">
                      ...
                      <p className="text-xs text-slate-400">item{count !== 1 ? "s" : ""} sold</p>
                    </div>
                  ))}
                </div>
              </div>
                          ...
                          {dayOrders.map((order, idx) => {
                            ...
                            return (
                              <tr key={order.id} className={cn("border-b border-slate-100 last:border-0", idx % 2 === 1 ? "bg-slate-50/50" : "")}>
                                ...
                                <td className="px-4 py-3 text-slate-500">{totalItems} item{totalItems !== 1 ? "s" : ""}</td>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              <div className="rounded-xl bg-blue-50 border border-blue-100 shadow-sm px-5 py-4">
                ...
                <p className="text-xs text-blue-400">{weekOrders} orders total</p>
              </div>
              <div className="rounded-xl bg-indigo-50 border border-indigo-100 shadow-sm px-5 py-4">
              ...
              <div className="rounded-xl bg-emerald-50 border border-emerald-100 shadow-sm px-5 py-4">
                ...
                <p className="text-xs text-emerald-400">{weekDays.reduce((s, d) => s + d.itemsSold, 0)} items sold</p>
              </div>
              <div className="rounded-xl bg-violet-50 border border-violet-100 shadow-sm px-5 py-4">
                <p className="text-xs font-medium text-violet-600 uppercase tracking-wide">Total Rentals</p>
                <p className="text-2xl font-bold text-violet-700 mt-1">{formatCurrency(weekDays.reduce((s, d) => s + d.rentals, 0))}</p>
                <p className="text-xs text-violet-400">{weekDays.reduce((s, d) => s + d.itemsRented, 0)} items rented</p>
              </div>
            </div>
