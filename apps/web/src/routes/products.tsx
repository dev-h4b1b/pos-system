    ...
    <div className="flex flex-col h-full overflow-hidden">
      ...
      {showManageGroups && (
        <ManageGroupsModal
          ...
          products={products}
          onAdd={addCategory}
          onRemove={(name) => { if (category === name) setCategory("all"); removeCategory(name); }}
    ...
    <div className={cn(
      ...
      <div>
        ...
        <p className={cn("font-semibold mt-0.5 text-sm", isRental ? "text-violet-600" : "text-blue-600")}>
          {formatCurrency(product.price)}{isRental ? "/day" : ""}
        </p>
      </div>
...
function ProductFormModal({ product, categories, onSave, onClose }: ProductFormModalProps) {
  ...
  const derivedStock = type === "rental" && units.length > 0 ? availableUnits : parseInt(stock) || 0;

  const isValid = name.trim().length > 0 && parseFloat(price) > 0 && derivedStock >= 0 && category.length > 0;

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isValid) return;
    const finalStock = type === "rental" && units.length > 0 ? availableUnits : parseInt(stock);
    onSave({ name: name.trim(), price: parseFloat(price), stock: finalStock, category, emoji, type, units: type === "rental" ? units : undefined });
  };
...
function AddStockModal({ product, onClose }: { product: Product; onClose: () => void }) {
  ...
  const handleSave = () => {
    if (isUnitTracked) {
      ...
      restockProduct(product.id, parseInt(qty) || 0);
    }
    onClose();
  };

  const canSave = isUnitTracked ? newUnitsAdded > 0 : (parseInt(qty) || 0) > 0;
        ...
        <div className="overflow-y-auto flex-1 px-6 py-5 space-y-4">
          ...
          <div className="flex items-center gap-3 rounded-xl bg-slate-50 border border-slate-100 px-4 py-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-slate-700">{product.stock} unit{product.stock !== 1 ? "s" : ""} available</p>
            </div>
          </div>
                ...
                <div className="space-y-2">
                  ...
                  {(parseInt(qty) || 0) > 0 && (
                    <p className="text-xs text-emerald-600 font-medium">
                      New stock: {product.stock + (parseInt(qty) || 0)} units
                    </p>
                  )}
                </div>
              )}
          ...
          {categories.map(cat => {
            ...
            return (
              <div key={cat} className="flex items-center gap-3 rounded-lg border border-slate-100 bg-slate-50 px-3 py-2.5">
                ...
                <span className="text-xs text-slate-400">{count} product{count !== 1 ? "s" : ""}</span>
                <button
